...
💪 Hãy ủng hộ hành trình cai thuốc của tôi!
  ...
  // Load quit plan templates when cigarettes per day changes
  const loadPlans = async () => {
    try {
      setLoading(true);
      setError(null);
      const templates = await getQuitPlanTemplates(formData.cigarettesPerDay);
      setPlans(templates);
    } catch (err) {
      setError('Không thể tải kế hoạch. Vui lòng thử lại sau.');
  ...
  // Load plans when cigarettes per day changes
  useEffect(() => {
    if (currentStep === 2) {
      loadPlans();
    }
  ...
  const renderQuitPlanOptions = () => {
    if (loading) {
      return <div>Đang tải kế hoạch...</div>;
    }
    ...
    if (!plans || plans.length === 0) {
      return <div>Không có kế hoạch phù hợp.</div>;
    }

    return (
      <div className="quit-plans">
        {plans.map((plan) => (
          <div
            key={plan.type}
            className={`plan-card ${formData.selectedPlan?.type === plan.type ? 'selected' : ''}`}
            onClick={() => handlePlanSelect(plan)}
          >
            <h3>{plan.name}</h3>
            <p className="description">{plan.description}</p>
            <div className="plan-details">
              <p>Thời gian: {plan.totalWeeks} tuần</p>
              <p>Giảm: {plan.reductionPerWeek}/tuần</p>
            </div>
            <div className="weekly-targets">
              {plan.weeks.map((week, index) => (
                <div key={week.week} className="week-target">
                  <span>Tuần {week.week}:</span>
                  <span>{week.target} điếu/ngày</span>
                </div>
              ))}
            </div>
            <p className="recommendation">{plan.recommendedFor}</p>
          </div>
        ))}
      </div>
    );
